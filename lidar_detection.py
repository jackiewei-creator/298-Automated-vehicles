...
import numpy as np
from opencda.customize.core.sensing.perception.util import draw_ground, draw_cluster, array_to_aabb_list
...
def remove_ego(point_cloud: o3d.geometry.PointCloud):
    ...
    y = points[:, 1]
    z = points[:, 2]
    mask = (x < -3) | (x > 3) | (y < -1.5) | (y > 1.5) | (z < -2) | (z > 1.5)
...
def clustering(point_cloud: o3d.geometry.PointCloud,
    ...
    clusters = np.array(point_cloud.cluster_dbscan(eps=0.5,
                                                   min_points=min_cluster_size,
                                                   print_progress=False))
    # END TODO

    # remove noise points that do not belong to any cluster. Those noise
